...
def trade_ticker(row, run_type):
    ...
    if run_type == 'trading':
        ...
        file_path = os.path.join(DIR, 'data', f'{ticker}.dat')
        if not os.path.exists(file_path):
            minute_data = dl_polygon_single(ticker, ndays=past_days)
            minute_data.to_csv(f'{ticker}_minute.csv')
            pickle.dump(minute_data, open(file_path, 'wb'))

        else:
            minute_data = pickle.load(open(file_path, 'rb'))
            minute_data.to_csv(f'{ticker}_minute.csv')
        ...
        df_returns = calc_returns_new(minute_data,
                                      ...
                                      effect_end, effect_end_days, period, lookback)
        ...
        hist_ivs = []
        closes = get_closes(ticker, tradier)
        for opt in options.index:
            print(opt)
            iv = get_historical_iv(tradier, opt, past_days, chain, options, closes)
            if type(iv)!=int:
                hist_ivs.append(iv)
...
async def enter_trade(row):
    print(f'Waiting to enter trade at {dt.datetime.now()}')
    try:
        trade_ticker(row, run_type='trading')
    except Exception as e:
        traceback.print_exc()
        print(f'Some error for {row["Symbol"]}')
    else:
        print(f'Entering Trade for {row["Symbol"]} at {dt.datetime.now()}')


async def exit_trade(symbol):
    print(f'Waiting to exit trade at {dt.datetime.now()}')
    try:
        trade_ticker(row, run_type='exiting')
    except Exception as e:
        traceback.print_exc()
        print(f'Some error for {row["Symbol"]}')
    else:
        print(f'Exiting Trade for {row["Symbol"]} at {dt.datetime.now()}')
...
if __name__ == '__main__':
    ...
    if not check_file.exists() or 1:
        ...
        for i, row in inputs.iterrows():
            trade_ticker(row, run_type='trading')
            break
            # loop.create_task(run_at(row['effect_start'], enter_trade(row)))
            # loop.create_task(run_at(row['effect_end'], exit_trade(row)))
